...
const createCustomerNegotiation = asyncHandler(async (req, res) => {
    ...
    if (![QUOTATION_STATUSES.SENT_TO_CUSTOMER, QUOTATION_STATUSES.READY_FOR_CUSTOMER, QUOTATION_STATUSES.APPROVED, QUOTATION_STATUSES.UNDER_NEGOTIATION].includes(quotation.status)) {
        throw new ApiError(400, 'Quotation is not open for negotiation');
    }

    const normalizedMessages = normalizeMessages(req.body);
    const referencedLineIds = normalizedMessages
    ...
    const messages = await NegotiationMessage.insertMany(normalizedMessages.map((message) => ({
    ...
    if (quotation.status !== QUOTATION_STATUSES.UNDER_NEGOTIATION) {
        await createQuotationVersionSnapshot(quotation._id, {
            actor: req.user,
            reason: 'Customer negotiation opened'
        });

        await transitionQuotationState(quotation, QUOTATION_STATUSES.UNDER_NEGOTIATION, {
            actor: req.user,
            reason: req.body.reason || 'Customer negotiation submitted',
    ...
    return res
    .status(201)
    .json(new ApiResponse(201, {negotiation, messages}, 'Customer negotiation submitted successfully'));
});
...
const proposeCustomerDiscount = asyncHandler(async (req, res) => {
    ...
    if (![QUOTATION_STATUSES.APPROVED, QUOTATION_STATUSES.READY_FOR_CUSTOMER, QUOTATION_STATUSES.SENT_TO_CUSTOMER, QUOTATION_STATUSES.UNDER_NEGOTIATION].includes(quotation.status)) {
    ...
    const negotiation = await Negotiation.create({
        ...
        requests: [{
            ...
            comment: req.body.message || `Customer proposed ${proposedDiscountPercent}% discount`,
    ...
    const message = await NegotiationMessage.create({
        ...
        message: req.body.message || `Proposed ${proposedDiscountPercent}% discount`,
        proposedValue: {
            scope,
    ...
    if (quotation.status !== QUOTATION_STATUSES.UNDER_NEGOTIATION) {
        await createQuotationVersionSnapshot(quotation._id, {
            actor: req.user,
            reason: 'Customer discount proposal opened negotiation'
        });

        await transitionQuotationState(quotation, QUOTATION_STATUSES.UNDER_NEGOTIATION, {
            actor: req.user,
            reason: 'Customer proposed discount change',
            metadata: {negotiationId: negotiation._id}
        });
    }
    ...
    return res
    .status(201)
    .json(new ApiResponse(201, {negotiation, message}, 'Discount proposal submitted successfully'));
});
...
const acceptNegotiation = asyncHandler(async (req, res) => {
    ...
    if (req.user.role === USER_ROLES.SALES_REP && quotation.ownerId.toString() !== req.user.id.toString()) {
    ...
    quotation = await prepareQuotationForMaterialChange(quotation, {
        actor: req.user,
        reason: req.body.reason || 'Customer negotiation accepted'
    });
    ...
    .json(new ApiResponse(200, {
        ...
        risk,
        approvalDecision
    }, 'Negotiation accepted and quotation terms updated successfully'));
});
...
const rejectNegotiation = asyncHandler(async (req, res) => {
    ...
    return res
    .status(200)
    .json(new ApiResponse(200, {negotiation}, 'Negotiation rejected successfully'));
});
