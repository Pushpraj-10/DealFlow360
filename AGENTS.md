...
When a merge, pull, rebase, or cherry-pick produces conflicts:

1. Automatically inspect all conflicted files.
2. Understand the changes from both sides before resolving conflicts.
3. Preserve both changes when they are compatible.
4. If changes conflict semantically, choose the implementation that best matches the current project requirements and architecture.
5. Do not blindly choose `ours` or `theirs`.
6. Remove all conflict markers:
   - `<<<<<<<`
   - `=======`
   - `>>>>>>>`
7. Run the relevant build, tests, or type checks after resolution.
8. Fix any compilation/runtime issues caused by the merge.
9. Run `git status` and verify there are no remaining unmerged files.
10. Complete the merge/rebase operation.

Do not abort the merge just because conflicts are difficult. Only abort if explicitly instructed by the user.