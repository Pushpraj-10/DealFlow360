...
const ITEM = {
  ...
  orders: { label: 'Orders', href: '/operations/orders', icon: <ClipboardList size={size} /> },
...
const NAV_BY_ROLE: Record<string, NavGroup[]> = {
  ...
  FINANCE: [
    { label: 'Workspace', items: [ITEM.overview, ITEM.approvals, ITEM.orders, ITEM.fulfillment, ITEM.subscriptions, ITEM.invoices, ITEM.payments] },
  ],
  ADMIN: [
    { label: 'Overview', items: [ITEM.overview] },
    { label: 'Sales', items: [ITEM.customers, ITEM.products, ITEM.pricing] },
    { label: 'Operations', items: [ITEM.orders, ITEM.fulfillment, ITEM.backorders, ITEM.warehouses, ITEM.inventory, ITEM.subscriptionPlans] },
